...
# Grab required data from environment
  ...
  $teamsAnonDialinWaitInLobby = Get-CsTeamsMeetingPolicy | Where-Object {$_.AllowPSTNUsersToBypassLobby -eq $true -and $_.AutoAdmittedUsers -eq "Everyone"}
    ...
    foreach ($_ in $teamsAnonDialinWaitInLobby) {
      if ($_.AllowPSTNUsersToBypassLobby -eq $true) {
        $htmlConstruction += $policyLoop = [ordered] @{
          ...
          'Dail-In Users Can bypass lobby' = $_.AllowPSTNUsersToBypassLobby
...
# Create Html Report
  New-HTML {
    ...
    New-HtmlSection -HeaderText 'Anonymous & Dail-In Users Meetings Settings' {
      New-HTMLTable -DataTable $htmlConstruction -HideFooter -Transpose -DisableInfo
    }
  } -FilePath ".\Reports\$CurrentReportFolderName\HtmlReports\AnonAndDialInBypassLobby.html"
}
