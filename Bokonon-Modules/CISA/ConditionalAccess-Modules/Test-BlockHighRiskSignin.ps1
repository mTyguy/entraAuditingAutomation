...
#>
function Test-BlockHighRiskSignin {
  [Cmdletbinding()]
  Param(
  )
...
# Set default to Fail
  ...
  foreach ($_ in $conditionalAccessPolicies) {
    if ($_.conditions.signInRiskLevels -match "high" -and $_.grantControls.builtInControls -eq "block" -and $_.state -eq "enabled") {
...
# Create Rule Meta Data
  $htmlRuleMetaData = New-RuleMetaData -RuleDescription "High Risk Signins Shall be Blocked." -Result "$PassFail" -Resolution "Ensure there is a Conditional Access policy that Blocks login events that are determined to be High risk" -Controls "Use these technologies to remediate" -Citations "https://learn.microsoft.com/en-us/entra/id-protection/howto-identity-protection-configure-risk-policies" -Framework "Nist SP 800-53 Rev 5 FedRAMP High"
  ...
  foreach ($_ in $conditionalAccessPolicies) {
    if ($_.conditions.signInRiskLevels -match "high" -and $_.grantControls.builtInControls -eq "block" -and $_.state -eq "enabled") {
      $htmlConstruction += $resultArray = [ordered] @{
        ...
        'Signin Risk Levels'      = ($_.conditions.signInRiskLevels) -join ", "
...
# Create Html Report
  New-HTML {
    New-HtmlSection -HeaderText 'MS.AAD.2.3 - High Risk Signins Shall be Blocked.' {
