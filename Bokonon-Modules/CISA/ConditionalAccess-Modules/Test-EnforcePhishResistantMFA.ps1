...
#>
function Test-EnforcePhishResistantMFA {
  [Cmdletbinding()]
  Param(
  )
...
# Create Rule Meta Data
  $htmlRuleMetaData = New-RuleMetaData -RuleDescription "Phishing Resistant MFA Shall be Enabled for all Users." -Result "$PassFail" -Resolution "Ensure there is a Conditional Access policy that enforces phishing resistant MFA for all users" -Controls "Use these technologies to remediate" -Citations "https://learn.microsoft.com/en-us/entra/identity/conditional-access/policy-all-users-mfa-strength" -Framework "Nist SP 800-53 Rev 5 FedRAMP High"

# Environment Data Array
  $htmlConstruction = @()

  foreach ($_ in $conditionalAccessPolicies) {
    if ($_.grantControls.authenticationStrength.displayName -eq "Phishing-resistant MFA" -and $_.conditions.clientAppTypes -eq "all" -and $_.conditions.users.includeUsers -eq "All") {
...
# Create Html Report
  New-HTML {
    New-HtmlSection -HeaderText 'MS.AAD.3.1 - Phishing Resistant MFA Shall be Enabled for all Users' {
