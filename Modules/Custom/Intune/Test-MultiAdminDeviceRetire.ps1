...
#>
function Test-MultiAdminDeviceRetire {
  [Cmdletbinding()]
  Param(
  )
...
# Set default to Fail
  ...
  foreach ($_ in $intuneMultiAdminApprovalPolicies) {
    if ($_.policyType -eq "deviceRetire") {
      $PassFail = "Pass"
      break
    } else {
      $PassFail = "Fail"
...
# Create Rule Meta Data
  $htmlRuleMetaData = New-RuleMetaData -RuleDescription "Multiple Admins Should be required to Retire a device." -Result "$PassFail" -Resolution "Ensure multiple admins are required to Retire a device" -Controls "Use these technologies to remediate" -Citations "https://learn.microsoft.com/en-us/intune/intune-service/fundamentals/multi-admin-approval" -Framework "Custom Rule"
...
# Environment Data Array
  ...
  if ($PassFail -eq "Fail") {
    $htmlConstruction = [ordered] @{
      'Result'      = "Multiple Administrators are not required to Retire a device"
      ...
      foreach ($_ in $intuneMultiAdminApprovalPolicies) {
        if ($_.policyType -eq "deviceRetire") {
          $htmlConstruction += $loop = [ordered] @{
            ...
            'Result'       = "Multiple Administrators are required to Retire a device"
            }
          }
        }
      }
...
# Create Html Report
  New-HTML {
    New-HtmlSection -HeaderText 'Custom.Intune.Devices.1.02 - Multiple Admins Should be required to Retire a device' {
  ...
  } -FilePath ".\Reports\$CurrentReportFolderName\HtmlReports\Custom.Intune.Devices.1.02_MultiAdminDeviceRetire.html"
}
